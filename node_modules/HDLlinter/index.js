...
const Vsg = require('HDLlinter/linter/Vsg');

// Verilog or SystemVerilog only
const Icarus = require('HDLlinter/linter/Icarus');
const Verilator = require('HDLlinter/linter/Verilator');
const Verible = require('HDLlinter/linter/Verible');

// VHDL only
const Ghdl = require('HDLlinter/linter/Ghdl');

// HDL linter
const Vivado = require('HDLlinter/linter/Vivado');
const Modelsim = require('HDLlinter/linter/Modelsim');
...
class Linter {
    ...
    async lint(doc) {
        ...
        if (doc.languageId == "verilog") {
            errors = await this.vlog_linter.lint_from_file(
                current_path,
            ...
            errors = await this.vhdl_linter.lint_from_file(
                current_path,
    ...
    async lint_from_uri(uri, empty = false) {
        let current_path = uri.fsPath;
        let languageId = this.getLanguageId(current_path);
        let errors = [];
        ...
        else if (languageId == "systemverilog") {
            errors = await this.vlog_linter.lint_from_file(
                current_path,
                this.get_vlog_config(),
                this.vlog_linter.SVLOGPARAMETERS);
