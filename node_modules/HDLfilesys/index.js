...
class processPrj {
    ...
    getPropertyInfo() {
        ...
        if (!fs.existsSync(this.opeParam.propertyPath)) {
            if (fs.existsSync(`${this.opeParam.workspacePath}/property.json`)) {
                ...
                this.opeParam.prjStrcture.HardwareSrc = this.opeParam.workspacePath;
                return false;
            }
        }
        ...
        if (files.isHasValue(this.opeParam.prjInfo, "PRJ_STRUCTURE", "customer")) {
            this.opeParam.prjStrcture.prjPath = this.opeParam.prjInfo.PRJ_Path;

            this.opeParam.prjStrcture.HardwareSim  = this.opeParam.prjInfo.HardwareSim.replace(/\\/g, "\/").replace("${workspace}", this.opeParam.workspacePath);
            this.opeParam.prjStrcture.HardwareSrc  = this.opeParam.prjInfo.HardwareSrc.replace(/\\/g, "\/").replace("${workspace}", this.opeParam.workspacePath);
            this.opeParam.prjStrcture.HardwareData = this.opeParam.prjInfo.HardwareData.replace(/\\/g, "\/").replace("${workspace}", this.opeParam.workspacePath);

            this.opeParam.prjStrcture.SoftwareSrc  = this.opeParam.prjInfo.SoftwareSrc.replace(/\\/g, "\/").replace("${workspace}", this.opeParam.workspacePath);
            this.opeParam.prjStrcture.SoftwareData = this.opeParam.prjInfo.SoftwareData.replace(/\\/g, "\/").replace("${workspace}", this.opeParam.workspacePath);
            return false;
        }

        // CN: 根据片上系统的类型获取标准工程结构
        this.opeParam.prjStrcture.prjPath = `${this.opeParam.workspacePath}/prj`;
        ...
        if (!files.isHasValue(this.opeParam.prjInfo, "SOC_MODE.soc", "none")) {
            ...
            this.opeParam.prjStrcture.SoftwareSrc  = `${this.opeParam.workspacePath}/user/Software/src`;
            this.opeParam.prjStrcture.SoftwareData = `${this.opeParam.workspacePath}/user/Software/data`;
        }
        this.opeParam.prjStrcture.HardwareSrc  = `${srcPath}/src`;
        this.opeParam.prjStrcture.HardwareSim  = `${srcPath}/sim`;
        this.opeParam.prjStrcture.HardwareData = `${srcPath}/data`;
    ...
    async processPrjFiles(ignoreCheck) {
        ...
        console.log(this.indexer.symbols);
    ...
    async refreshPrjFolder() {
        ...
        if (files.isHasAttr(this.opeParam.prjInfo, "SOC_MODE.soc")) {
            if (this.opeParam.prjInfo.SOC_MODE.soc == "none") {
                if (fs.existsSync(`${this.opeParam.workspacePath}/user/Software`)) {
                    if (this.setting.get("PRJ.file.structure.notice")) {
                        let select = await this.warn("Software will be deleted.", 'Yes', 'No');
                        if (select == "Yes") {
                            dirs.rmdir(`${this.opeParam.workspacePath}/user/Software`);
                        }
                    } else {
                        dirs.rmdir(`${this.opeParam.workspacePath}/user/Software`);
                    }
                }
                if (fs.existsSync(`${this.opeParam.workspacePath}/user/Hardware`)) {
                    let fileList = fs.readdirSync(`${this.opeParam.workspacePath}/user/Hardware`);
                    for (let i = 0; i < fileList.length; i++) {
                        const element = fileList[i];
                        dirs.mvdir(`${this.opeParam.workspacePath}/user/Hardware/${element}`, `${this.opeParam.workspacePath}/user`);
                    }
                    dirs.rmdir(`${this.opeParam.workspacePath}/user/Hardware`);
                } else {
                    dirs.mkdir(`${this.opeParam.workspacePath}/user/src`);
                    dirs.mkdir(`${this.opeParam.workspacePath}/user/sim`);
                    dirs.mkdir(`${this.opeParam.workspacePath}/user/data`);
                }
            } else {
                if (fs.existsSync(`${this.opeParam.workspacePath}/user`)) {
                    let fileList = fs.readdirSync(`${this.opeParam.workspacePath}/user`);
                    if (!fs.existsSync(`${this.opeParam.workspacePath}/user/Hardware`)) {
                        dirs.mkdir(`${this.opeParam.workspacePath}/user/Hardware`);
                        for (let i = 0; i < fileList.length; i++) {
                            const element = fileList[i];
                            dirs.mvdir(
                                `${this.opeParam.workspacePath}/user/${element}`, 
                                `${this.opeParam.workspacePath}/user/Hardware`
                            );
                        }
                    }
                } else {
                    dirs.mkdir(`${this.opeParam.workspacePath}/user/Hardware/src`);
                    dirs.mkdir(`${this.opeParam.workspacePath}/user/Hardware/sim`);
                    dirs.mkdir(`${this.opeParam.workspacePath}/user/Hardware/data`);
                }
                dirs.mkdir(`${this.opeParam.workspacePath}/user/Software/data`);
                dirs.mkdir(`${this.opeParam.workspacePath}/user/Software/src`);
            }
        }
    }
    ...
    getPrjFiles(HDLFileList) {
        ...
        if (this.opeParam.prjInfo) {
            ...
            this.getHDLFiles(this.opeParam.prjStrcture.HardwareSim, HDLFileList);
        ...
        // CN: 获取src下的全部HDL文件
        this.getHDLFiles(this.opeParam.prjStrcture.HardwareSrc, HDLFileList);
    }
    ...
    getXilinxFiles(HDLFileList) {
        let bd_path = `${fspath.dirname(this.opeParam.prjStrcture.HardwareSrc)}/bd`;
    ...
    async processLibFiles() {
        let localLibPath  = `${this.opeParam.prjStrcture.HardwareSrc}/lib`;
        ...
        if (files.isHasValue(this.opeParam.prjInfo, "HardwareLIB.State", "real")) {
            ...
            for (let i = 0; i < delLibFileList.length; i++) {
                ...
                this.indexer.symbols.delete(elementPath);
            }
        
            this.oldLibFileList = this.newLibFileList;
            
            // 向HDLparam和HDLsymbol中添加新lib文件
            await this.indexer.build_index(addLibFileList);
            if (this.treeView) {
    ...
    monitorHDL() {
        ...
        // 添加监听事件
        this.watcherHDL.on('add', async (path) => {
        ...
        this.watcherHDL.on('change', async (path) => {
            path = path.replace("//","/").replace(/\\/g,"\/");
            indexer.removeCurrentFileParam(path);
            indexer.symbols.delete(path);
            await indexer.processFile(vscode.Uri.file(path), indexer.HDLparam);
...
exports.processPrj = processPrj;

/**
 * 注册工程相关服务，全为函数无 class
 * @param {*} opeParam 操作变量
 */
function registerPrjsServer(opeParam){
    ...
    vscode.commands.registerCommand('HARD.Add_dev', () => {
        prjs.addDevice(opeParam);
    });
    vscode.commands.registerCommand('HARD.Delete_dev', () => {
