...
function generatetbFile(opeParam) {
    ...
    vscode.window.showQuickPick(
        ['vlog','vhdl'],
        {placeHolder:"Which type you want to generate?"}
    ).then(select => {
...
function genInstancetoTbFile(indexer, tbFilePath) {
    ...
    let docPath = vscode.window.activeTextEditor.document.uri.fsPath;
    parser.utils.selectCurrentFileModule(indexer.HDLparam, docPath).then(selectModule => {
        if (selectModule != null) {
            if (doc.languageId == "systemverilog" || doc.languageId == "verilog") {
                ...
                insertInstContent(inst, tbFilePath);
...
function insertInstContent(content, tbFilePath) {
    let newContent = "";
    let oldContent = fs.readFileSync(tbFilePath, "utf-8");
