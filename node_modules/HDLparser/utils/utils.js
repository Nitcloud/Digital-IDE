...
function refreshInstModulePath (HDLparam) {
    ...
    for (let i = 0; i < HDLparam.length; i++) {
        ...
        // 遍历每个模块下的所有例化模块
        for (let x = 0; x < unitMoudule.instmodule.length; x++) {
            const unitInstanceModule = unitMoudule.instmodule[x];
            // 从HDLparam中找到每个例化模块的路径
            for (let z = 0; z < HDLparam.length; z++) {
                const element = HDLparam[z];
                if (unitInstanceModule.instModule == element.moduleName) {
