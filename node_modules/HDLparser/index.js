...
const sm_parser  = require("./sm_paeser/sm_parser");

exports.utils  = utils;
exports.vlogParser = vlogParser;
exports.vhdlParser = vhdlParser;

exports.sm_parser  = sm_parser;

const ParserLib = require("./utils/factory");

async function getHDLFileParam(document, HDLparam, HDLSymbol) {
    let symbolList = [];
    let parser = new ParserLib.ParserFactory;
    let lang_parser = await parser.getParser(document.languageId);
    let structure = lang_parser.get_all(text);
    for (let i = 0; i < structure.length; i++) {
        const module = structure[i];
        let HDLfileparam = {
            "moduleName" : module,
            "modulePath" : document.uri._fsPath.replace(/\\/g,"\/"),
            "instmodule" : [],
            "param"      : [],
            "port"       : {
                "inout"  : [],
    ...
    let symbolInfo = HDLSymbol.setSymbolInformationWithRegExp(match, parent, document, offset);
...
class HDLSymbol {
     ...
     * @return The object of the SymbolInformation.
     */
     setSymbolInformationWithTreeSitter(match, containerName, document, offset){
        let location = new vscode.Location(
            document.uri, 
            new vscode.Range(
                document.positionAt(match.index + offset), 
            ...
            return yield vscode.window.withProgress({
                ...
                for (var filenr = 0; filenr < FileList.length; filenr += this.parallelProcessing) {
                    ...
                    yield Promise.all(subset.map((uri) => {
                        return this.processFile(vscode.Uri.file(uri), FileList.length, this.HDLparam);
                    }));
                }
            })).then(() => {
