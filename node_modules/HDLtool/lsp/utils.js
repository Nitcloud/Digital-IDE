...
var utils = {
    ...
    getSymbols : function (document) {
        ...
        switch (document.languageId) {
            case "verilog":
            case "systemverilog":
                parser.vlogParser.getFileParam(option); // 获取所有的标识的全部信息
            break;
            case "vhdl":
                parser.vhdlParser.getFileParam(path, option); // 获取所有的标识的全部信息
            break;
            default: return[];
        }
    ...
    getSymbolDefine : function (document, position) {
        ...
        switch (document.languageId) {
            case "verilog":
            case "systemverilog":
                parse = parser.vlogParser;
            break;
            case "vhdl":
                parse = parser.vhdlParser;
            break;
            default: return[];
        }

        let word  = document.getText(range);  // 获取到标识字符
        let text  = document.getText(); 
        // 标识字符的起始位置序号和终止位置序号的对象
        let wordIndex = {
            startIndex : document.offsetAt(range.start),
            lastIndex  : document.offsetAt(range.end),
        };

        let guide = '';
        if (wordIndex.startIndex != 0) {
            guide = text[wordIndex.startIndex-1];
        }
        ...
        for (let i = 0; i < illegalRange.length; i++) {
            ...
            if (this.ensureInclude(index, wordIndex)) {
                return [];
            }
        }
        ...
        symbol.symbols = [];
        symbol.curdoc  = document;
        parse.getFileParam(path, option);
        switch (guide) {
            // 1. instance's port/param : 哪个instance -> 对应的module -> 定义的位置
            case '.': 
                let item = this.getItemInfo(wordIndex, symbol.symbols, "instance");
                if (item) {
                    let instProperty = this.getInstDefine(wordIndex, item);
            ...
            // 其他标识
            default:
                this.getWordInfo(word, wordIndex, symbol.symbols, results);
            break;
        }
     ...
     * @param {*} wordIndex 标识符所在的位置 index表示形式
     * @param {*} item 被例化的模块的详细属性
     * @returns 返回标识符所在的位置找到被定义内容(port | param)
     */
    getInstDefine : function (wordIndex, item){
        ...
        // 确认所需定位的标识是否为例化的端口
        let results = null;
        parse.get_methods(content);
        if (this.ensureInclude(item.instports, wordIndex)) {
            results = parse.get_ports(content);
        }
        
        // 确认所需定位的标识是否为例化的参数
        if (this.ensureInclude(item.instparams, wordIndex)) {
            results = parse.get_params(content);
        }

        if (results) {
            for (let index = 0; index < results.length; index++) {
                const element = results[index];
                let range = this.index_to_range(text, {
     ...
     * @param {Object} wordIndex 标识符所在的范围 index表示方式
     * @param {Array}  symbols   该文件下的所有的标识符信息组
     * @param {Array}  results   标识符被定义的结果results = { uri : uri, range : range, }
     */
    getWordInfo : function (word, wordIndex, symbols, results) {
        // 优先检测是否是指定例化模块名module
        let item = this.getItemInfo(wordIndex, symbols, "instance");
        if (item.instModule == word) {
            for (let index = 0; index < this.HDLparam.length; index++) {
                ...
                // 如果是则直接转换地址范围并导出
                let text = filesys.files.readFile(element.modulePath);
                let range = this.index_to_range(text, {
                    startIndex : element.startIndex,
                    lastIndex  : element.lastIndex,
                });
        ...
        for (let index = 0; index < symbols.length; index++) {
            ...
            if (symbolElement.languageId != 'vhdl') {
                let itemInfo = this.getItemInfo(wordIndex, symbols, "module");
                // 判断这个定义是否在同一个module下
     ...
     */
    getItemInfo : function (wordIndex, symbols, itemName) {
        for (let index = 0; index < symbols.length; index++) {
            const symbolElement = symbols[index];
            ...
            if (this.ensureInclude(symbolElement.item, wordIndex)) {
