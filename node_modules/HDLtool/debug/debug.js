...
class toolRegister {
    ...
    serialPortTerminal(serialPortName,command) {
        ...
        // });
        if (ensureTerminalExists(`${serialPortName}`)) {
    ...
    runSerialPort(command) {
        var _this = this;
        let porteries  = `${this.BaudRate} ${this.DataBits} ${this.StopBits} ${this.Parity}`;
        let scriptPath = `${this.opeParam.rootPath}/../resources/tool/.Script/Serial_Port.py`
        child.exec(command,function (error, stdout, stderr) {
            ...
            if (SerialPortList[0] == "one") {
                ...
                let command = `python ${scriptPath} runthread ${porteries}`;
                _this.serialPortTerminal(SerialPortList[1],command);
            }
            if (SerialPortList[0] == "multiple") {
                ...
                vscode.window.showQuickPick(SerialPortList).then(selection => {
                    ...
                    let command = `python ${scriptPath} runthread ${porteries}`;
                    _this.serialPortTerminal(selection,command);
                });
            }
            if (error !== null) {
