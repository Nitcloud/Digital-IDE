...
async function selectModule (HDLparam, workspace_path) {
    const selectModule = await vscode.window.showQuickPick(refreshModuleInfoList(HDLparam, workspace_path));
    if (!selectModule) {
        return;
    } else {
        var moduleList = [];
...
async function selectCurrentFileModule (HDLparam, doc) {
    ...
    HDLparam.forEach(element => {
        if (element.modulePath == doc.fileName) {
            moduleNameList.push(element.moduleName);
        }
    });
    const selectModule = await vscode.window.showQuickPick(moduleNameList);
    if (!selectModule) {
        return;
    } else {
        var moduleList = [];
