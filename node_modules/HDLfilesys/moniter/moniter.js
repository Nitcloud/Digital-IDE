...
function monitor(path, opeParam, indexer, outputChannel, callback) {
    ...
    // let processLib = new prjs.processLib(opeParam, watcher);
    // 添加监听事件
    watcher.on('add', (path) => {
        onCreate(path, indexer, callback, opeParam);
        console.log(`File ${path} has been added`);
    });
    watcher.on('change', (path) => {
        onChange(path, indexer, processLib, callback, opeParam, outputChannel);
        console.log(`File ${(path)} has been changed`);
    });
    watcher.on('unlink', (path) => {
        onDelete(path, indexer, callback);
...
async function onCreate(path, indexer, callback, opeParam) {
    try {
        if (path.indexOf("property.json") != -1) {
            processPropertyFile(opeParam, indexer, processLib);
        }
        else {
            await indexer.processFile(vscode.Uri.file(path), indexer.HDLparam);
            indexer.refreshInstModulePath();
        }
        callback();
    }
    catch (error) {
...
async function onChange(path, indexer, processLib, callback, opeParam, outputChannel) {
    try {
        ...
        if (path.indexOf("property.json") != -1) {
            ...
            indexer.HDLparam = indexer.removeCurrentFileParam(path);
            let fspath = path.replace(/\//g, "\\");
            indexer.symbols.delete(fspath);
            await indexer.processFile(vscode.Uri.file(path), indexer.HDLparam);
            indexer.refreshInstModulePath();
            callback();
        }
...
function onDelete(path, indexer, callback) {
    try {
        if (path.indexOf("property.json") != -1) {
...
async function processPropertyFile(opeParam, indexer, processLib) {
    prjs.refreshPropertyInfo(opeParam);
    prjs.refreshPrjFolder(opeParam);
    processLib.getLibParam();
    let libParam = processLib.processLibParam();
    console.time('parsetimer');
    for (let i = 0; i < libParam.del.length; i++) {
        const elementPath = libParam.del[i];
        indexer.HDLparam = indexer.removeCurrentFileParam(elementPath);
        let fspath = elementPath.replace(/\//g, "\\");
        indexer.symbols.delete(fspath);
    }
    await Promise.all(libParam.add.map((elementPath) => {
