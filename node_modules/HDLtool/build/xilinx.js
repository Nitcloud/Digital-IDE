...
class xilinxOperation {
    ...
    launch() {
        ...
        let scripts = [];
        let prjFiles = [];
        let prjFilePath = `${this.process.opeParam.prjStrcture.prjPath}/xilinx`;
    ...
    create(scripts) {
        ...
        scripts.push(`create_project ${prjName} ./prj/xilinx -part ${device} -force -quiet`);
    ...
    refresh() {
        ...
        let prjStrcture = opeParam.prjStrcture;
        let prjFilePath = `${opeParam.prjStrcture.prjPath}/xilinx`;
        let hardwarePath = path.dirname(prjStrcture.HardwareSrc);
        // 清除所有源文件
        ...
        // 导入本地设计源文件
        scripts.push(`add_files ${prjStrcture.HardwareSrc} -quiet`);

        // 导入本地仿真源文件
        scripts.push(`add_files -fileset sim_1 ${prjStrcture.HardwareSim} -quiet`);

        // 导入本地约束源文件
        scripts.push(`add_files -fileset constrs_1 ${prjStrcture.HardwareData} -quiet`);

        scripts.push(`set_property top ${opeParam.srcTopModule.path} [current_fileset]`);
        scripts.push(`set_property top ${opeParam.simTopModule.path} [get_filesets sim_1]`);
    ...
    build() {
        ...
        script += `launch_runs synth_1 -to_step write_bitstream -jobs 4 ${quietArg}\n`;
        script += `report_timing_summary ${quietArg}\n`;
        let scriptPath = this.generateBit(terminal);
        script += `source ${scriptPath} -notrace\n`;
    ...
    generateBit() {
        ...
        let softwareData = opeParam.prjStrcture.SoftwareData;
        let prjFilePath = `${opeParam.prjStrcture.prjPath}/xilinx`;
    ...
    async boot() {
        ...
        let opeParam = this.process.opeParam;
        let BootInfo = {
            "outsidePath" : `${path.dirname(opeParam.prjStrcture.HardwareSrc)}/boot`,
    ...
    clean() {
        let opeParam = this.process.opeParam;
        move_bd_ip();
        filesys.dirs.rmdir(opeParam.prjStrcture.prjPath); 
        
        let strFile = filesys.files.pickFile(opeParam.workspacePath,".str");
        strFile.forEach(element => {
            filesys.files.rmdir(`${opeParam.workspacePath}/${element}`);
        });
    ...
    move_bd_ip() {
        let opeParam = this.process.opeParam;
        let prj_info = opeParam.prjInfo;
        let target_path = path.dirname(opeParam.prjStrcture.HardwareSrc);
