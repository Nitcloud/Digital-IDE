...
var vlog_constants = new RegExp([
    /(?<=\s)/,
    /(?<type>localparam|genvar|integer)\s*/,
    // /(?!(,|=))/,
].map(x => (typeof x === 'string') ? x : x.source).join(''), 'mg');
...
/////////////////////////block api////////////////////////////////////
function get_modules(text, document, parent, HDLparam, HDLSymbol, type) {
    let items = [];
    while (1) {
        ...
        if (type != "simple") {
            let structure = get_module_elements(content, document, item.name, item.startIndex, HDLSymbol);
    ...
    // if (!items.length && type != "simple") {
    //     let content = {"text" : text};
    //     get_module_elements(content, document, null, 0, HDLSymbol);
    // }
    return items;
}
exports.get_modules = get_modules;

function get_module_elements(content, document, parent, offset, HDLSymbol) {
    // get blocks
    get_generates(content, document, parent, offset, HDLSymbol);
    get_methods(content, document, parent, offset, HDLSymbol);
    
    // get elements
    let structure = get_element(content, document, parent, offset, HDLSymbol);
    return structure;
}
//--------classes block--------//
...
/////////////////////////element api////////////////////////////////////
//--------get element--------//
function get_element(content, document, parent, offset, HDLSymbol) {
    let ports     = get_ports(content.text, document, parent, offset, HDLSymbol);
    let signals   = get_signals(content.text, document, parent, offset, HDLSymbol);
    let asserts   = get_asserts(content.text, document, parent, offset, HDLSymbol);
    let parameter = get_parameter(content.text, document, parent, offset, HDLSymbol);
    let instances = get_instances(content.text, document, parent, offset, HDLSymbol);
    let constants = get_constants(content.text, document, parent, offset, HDLSymbol);
    let processes = get_processes(content.text, document, parent, offset, HDLSymbol);
...
function get_include(text) {
    let items = [];
    while (1) {
        let match = vlog_include.exec(text);
        if (match == null) {
            break;      
        }
        items.push(match.groups.path);
    }
    return items;
}

/////////////////////////util api////////////////////////////////////
function get_balance_index(items, startItems, lastItems) {
