...
class fastVisitor extends vhdlListener.vhdlListener {
    ...
    enterComponent_instantiation_statement(ctx) {
        ...
        // 12 : architecture_body
        let arch = this.base.parent_traverse(ctx, 12);
        if (!arch) {
            return null;
        }
        let identifiers = arch.identifier();
        let parent = identifiers[1].getText();
        ...
        try {
            ...
            if (option.isFast) { 
                ...
                for (let index = 0; index < walkResult.modules.length; index++) {
                    ...
                    for (let i = 0; i < walkResult.instances.length; i++) {
                        const instance = walkResult.instances[i];
                        if (instance.parent != module.moduleName) {
                            continue;
                        }
