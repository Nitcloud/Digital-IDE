...
function Clean(opeParam) {
    // move_bd_IP(opeParam);
    filesys.dirs.deleteDir(`${opeParam.workspacePath}.Xil`);
    filesys.dirs.deleteDir(`${opeParam.workspacePath}prj`);
    let strFile = filesys.files.pickFile(opeParam.workspacePath,".str");
    strFile.forEach(element => {
        filesys.files.deleteFile(`${opeParam.workspacePath}${element}`);
    });
...
function Add_bd(opeParam) {
    ...
    if ( bd_folder == "") {
        bd_folder = `${opeParam.rootPath}/../lib/Ailinx_lib/bd`;
    }
...
function Delete_bd(opeParam) {
    let propertyParam = filesys.files.pullJsonInfo(`${opeParam.rootPath}/../property.json`);
    vscode.window.showQuickPick(filesys.files.pickFile(`${opeParam.rootPath}/../lib/Ailinx_lib/bd`,".bd")).then(selection => {
        ...
        filesys.files.pushJsonInfo(`${opeParam.rootPath}/../property.json`,propertyParam);
        const bd_path = `${opeParam.rootPath}/../lib/Ailinx_lib/bd` + selection;
...
function Overwrite_bd(opeParam) {
    vscode.window.showQuickPick(this.file.pick_file(`${opeParam.rootPath}/../lib/Ailinx_lib/bd`,".bd")).then(selection => {
        // the user canceled the selection
      if (!selection) {
          return;
      }
...
function setTopFile(indexer, callback) {
    let editor = vscode.window.activeTextEditor;
    if (!editor) {
        return null;
    }
    let current_fileset = "[current_fileset]";
    if (editor.document.fileName.replace(/\\/g,"\/").replace("//","/").indexOf("sim") != -1) {
        current_fileset = "[get_filesets sim_1]";
    }
    let moduleNameList = [];
    indexer.HDLparam.forEach(element => {
    ...
    if (moduleNameList == null) {
        ...
        if (moduleNameList.length > 1) {
            vscode.window.showQuickPick(moduleNameList).then(selection => {
                ...
                callback(`set_property top ${selection} ${current_fileset}`);
            });
        } else {
            // set_property top hexCounterTest [get_filesets sim_1]
            callback(`set_property top ${moduleNameList[0]} ${current_fileset}`);
        }
    }
}
exports.setTopFile = setTopFile;
