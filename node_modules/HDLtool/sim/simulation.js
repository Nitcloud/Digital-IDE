...
class iverilogSimulate {
    ...
    simulate() {
        ...
        let moduleNameList = [];
        for (let i = 0; i < this.HDLparam.length; i++) {
            const element = this.HDLparam[i];
        ...
        if (moduleNameList.length >= 1) {
            ...
            this.HDLparam.forEach(unitModule => {     
            ...
            // TODO 输出vvp路径错误  runFilePath设置问题
            let command = `${iVerilogPath} -g2012 -o ${iverilogPath} ${editor.document.fileName} ${rtlFilePath} ${this.GlblPath} ${this.LibPath}`;
