...
function instantiateModuleInteract(HDLparam, workspace_path) {
    var languageId = vscode.window.activeTextEditor.document.languageId;
    selectModule(HDLparam, workspace_path).then(selectModule => {
...
async function selectModule (HDLparam, workspace_path) {
    const selectModule = await vscode.window.showQuickPick(parser.simParser.refreshModuleInfoList(HDLparam, workspace_path));
    if (!selectModule) {
        return;
    } else {
        var moduleList = [];
        moduleList = parser.simParser.findModule(HDLparam, selectModule);
