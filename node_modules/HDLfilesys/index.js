...
class processPrjFiles {
    ...
    getConfig() {
        this.customerlocalLibPath = this.setting.get("PRJ.customer.Lib.repo.path");
    }
    ...
    async processPrjFiles(ignoreCheck) {
        ...
        await this.indexer.build_index(HDLFileList, 'local');
    ...
    getPrjFiles(HDLFileList) {
        ...
        if (this.opeParam.prjInfo) {
            ignores = this.opeParam.prjInfo.ignores;
        ...
        if (ignores) {
            for (let i = 0; i < ignores.length; i++) {
                ignores[i] = this.opeParam.workspacePath + '/' + ignores[i];
            }
        }

        // let hdlignores = files.readFile(this.opeParam.workspacePath + '/.hdlignore');
        // if (hdlignores) {
        ...
        // CN: 获取src下的全部HDL文件
        let srcPath = this.opeParam.prjStructure.HardwareSrc;
     ...
     * @param {*} HDLFileList 
     */
    getHDLFiles(path, HDLFileList, ignores) {
        let HDLFileExtnames = [
            // verilog
            ".v", ".V", ".vh", ".vl", 
            // systemverilog
        ...
        files.pickAllFile(path, HDLFileExtnames, HDLFileList, ignores);
    }
    ...
    getLibFiles() {
        ...
        if (files.isHasAttr(this.opeParam.prjInfo, "HardwareLIB.common")) {
            for (let i = 0; i < this.opeParam.prjInfo.HardwareLIB.common.length; i++) {
                ...
                this.getHDLFiles(`${this.localLibPath}/com/Hardware/${element}`, libFileList);
            }
        }

        // Get the HDL file of the local Xilinx library and put it in libFileList
        if (files.isHasAttr(this.opeParam.prjInfo, "HardwareLIB.xilinx")) {
            for (let i = 0; i < this.opeParam.prjInfo.HardwareLIB.xilinx.length; i++) {
                const element = this.opeParam.prjInfo.HardwareLIB.xilinx[i];
                this.getHDLFiles(`${this.localLibPath}/xilinx/src/${element}`, libFileList);
            }
        }

        // Get the HDL file of the local Intel library and put it in libFileList
        if (files.isHasAttr(this.opeParam.prjInfo, "HardwareLIB.intel")) {
            for (let i = 0; i < this.opeParam.prjInfo.HardwareLIB.intel.length; i++) {
                const element = this.opeParam.prjInfo.HardwareLIB.intel[i];
                this.getHDLFiles(`${this.localLibPath}/intel/src/${element}`, libFileList);
            }
        }
        ...
        if (files.isHasAttr(this.opeParam.prjInfo, "HardwareLIB.customer")) {
            ...
            if (fs.existsSync(this.customerlocalLibPath)) {
                for (let i = 0; i < this.opeParam.prjInfo.HardwareLIB.customer.length; i++) {
                    ...
                    this.getHDLFiles(`${this.customerlocalLibPath}/${element}`, libFileList);
                }
            } else {
