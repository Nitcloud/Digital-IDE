...
class DefinitionProvider {
    ...
    provideDefinition(document, position, token) {
        return new Promise((resolve, reject) => {
            ...
            let symbolPosition = this.position_to_index(text, range);
            let IllegalRange = parser.vlogParser.getCommentRange(text,0);
            if (parser.utils.isIllegalRange(symbolPosition, IllegalRange)) {
                resolve(results);
            }
            if (!range) {
                reject();
            ...
            if (symbols == undefined) {
                symbols = this.indexer.getFileParam(document, null, this.indexer.HDLSymbol);
            }
            let Symbol = {
                "word" : word,
                "startIndex" : symbolPosition.startIndex,
                "lastIndex"  : symbolPosition.lastIndex,
            }
            ...
            let modules = this.findModuleFromPath(this.indexer.HDLparam, path);
            for (let index = 0; index < modules.length; index++) {
                const module = modules[index];
                if (this.ensureInclude(module, symbolPosition)) {
                    currentModule = module;
                    modulePosition.startIndex = module.startIndex;
                    modulePosition.lastIndex  = module.lastIndex;
                    break;
                }
            }
            ...
            if (flage == '.') {
                this.getInstanceSymbol(currentModule, Symbol, results, "property");
                resolve(results);
            } else {
                ...
                for (let index = 0; index < symbols.length; index++) {
                    ...
                    if (symbolElement.name == word) {
                        let defsymbolPosition = this.position_to_index(text, symbolElement.location.range);
                        if (this.ensureInclude(modulePosition, defsymbolPosition)) {
                            let result = {
                ...
                this.getInstanceSymbol(currentModule, Symbol, results, "name");
                resolve(results);
            }
        });
    }
    position_to_index(content, position) {
        ...
        for (let i = 0; i < position.start.line; ++i) {
            startIndex = startIndex + lines[i].length + 1;
        }
        startIndex = startIndex + position.start.character;
        for (let i = 0; i < position.end.line; ++i) {
            lastIndex = lastIndex + lines[i].length + 1;
        }
        lastIndex = lastIndex + position.end.character;
        return {
    ...
    getInstanceSymbol(currentModule, Symbol, results, mode) {
        for (let index = 0; index < currentModule.instances.length; index++) {
            const instance = currentModule.instances[index];
            if (this.ensureInclude(instance, Symbol)) {
                ...
                for (let i = 0; i < modules.length; i++) {
                    ...
                    for (let x = 0; x < symbols.length; x++) {
                        ...
                        if (symbolElement.name == Symbol.word) {
