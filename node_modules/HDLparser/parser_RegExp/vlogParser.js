...
function get_modules(text, document, parent, HDLparam, HDLSymbol, type) {
    ...
    while (1) {
        ...
        let content = {"text" : match.groups.body};
        let offset  = match[0].indexOf(match.groups.body);
        if (type != "simple") {
            let structure = get_module_elements(content, document, item.name, item.startIndex + offset, HDLSymbol);
            if (match.groups.ports) {
                offset  = match[0].indexOf(match.groups.ports);
                structure.ports = structure.ports.concat(get_ports(match.groups.ports, document, item.name, item.startIndex + offset, HDLSymbol));
            }
            if (match.groups.params) {
                offset  = match[0].indexOf(match.groups.params);
                structure.parameter = structure.parameter.concat(get_parameter(match.groups.params, document, item.name, item.startIndex + offset, HDLSymbol));
            }

            let includeList = get_include(text, structure.modulePath);
            structure.includes   = includeList;
...
function get_module_elements(content, document, parent, offset, HDLSymbol) {
    ...
    // get elements
    let structure = get_element(content.text, document, parent, offset, HDLSymbol);
    return structure;
}
//--------classes block--------//
...
function get_include(text, curPath) {
    let items = [];
    while (1) {
        ...
        if (!fspath.isAbsolute(path)) {
            ...
            path = fspath.resolve(parent, path);
        }
        items.push(path.replace(/\\/g,"\/"));
    }
    return items;
}
exports.get_include = get_include;
...
function delComments(text) {
    let match = text.match(b_comment);
    if (match != null) {
