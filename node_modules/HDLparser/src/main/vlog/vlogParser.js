...
class visitor extends vlogListener.vlogParserListener {
    ...
    enterModule_instance(ctx) {
        ...
        this.sym[name] = this.curInstance;
    }
...
class parser {
    ...
    getFileParam(option, HDLparam) {
        try {
            ...
            this.walk(parseResult.ast, visit);
            let dir = filesys.paths.dirname(option.path);
            let name = filesys.paths.basename(option.path);
            filesys.files.pushJsonInfo(`${dir}/${name}.json`, visit.sym);
        }
        catch (error) {
            console.log(error);
            return HDLparam;
