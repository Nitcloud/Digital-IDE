...
class vlogCompletion {
    ...
    provideCompletionItems(document, position, token) {
        return new Promise((resolve, reject) => {
            ...
            let IllegalRange = parser.vlogParser.getCommentRange(content,0);
            content = parser.vlogParser.delComments(content);
            while (1) {
                // 匹配每个元素
                ...
                else if ((match.index == 0) || parser.utils.isIllegalRange(match,IllegalRange,0)) {
                    continue;   // 跳过匹配组中的一个
                }
