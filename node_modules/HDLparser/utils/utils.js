...
/**
 * 获取当前模块下的子模块
 * @param {*} moduleName 父级模块名
 * @param {*} HDLparam   全局HDL文件参数
 * @returns 该父级模块下所包含的所有例化模块信息
 */
function getChildElement(moduleName, HDLparam){
    let childElementList = [];
    // 先从全局HDL文件参数中找到该父级模块
    HDLparam.forEach(unitModule => {
        if (unitModule.moduleName == moduleName) {
            // 进行该模块下的例化模块的遍历
            for (let index = 0; index < unitModule.instances.length; index++) {
                const unitInstModule = unitModule.instances[index];
                let childElement = {
...
function getSrcTopElement(process, HDLparam, currentSrcTopElement) {
    let srcHDLparam       = [];
    let srcTopElementList = [];
    let srcPath = process.opeParam.prjStructure.HardwareSrc;

    for (let i = 0; i < HDLparam.length; i++) {
        const element = HDLparam[i];
        if (element.modulePath.includes(srcPath)) {
            srcHDLparam.push(element);
        }
    }

    srcTopElementList = getTopElement(srcHDLparam, currentSrcTopElement);
    let len = srcTopElementList.length;
    // 如果没有src文件则直接退出
    if (len == 0) {
        return srcTopElementList;
    }
    if (process.opeParam.srcTopModule.name == '') {
        // 如果没有就将第一个设置为当前顶层
        process.opeParam.srcTopModule = {
        ...
        for (let index = 0; index < len; index++) {
            ...
            if (process.opeParam.srcTopModule.name == simTopElement.type &&
                process.opeParam.srcTopModule.path == simTopElement.fsPath) {
                // 如果在第i个为当前顶层则将其删除
        ...
        if (isExsit) {
            let TopElement = {
                "mode"   : "src",
                "name"   : "",
                "type"   : process.opeParam.srcTopModule.name,
                "fsPath" : process.opeParam.srcTopModule.path
            };
            // 直接插入到顶层数组的第一个
...
function getSimTopElement(process, HDLparam, currentTbTopElement) {
    let simHDLparam       = [];
    let simTopElementList = [];
    let simPath = process.opeParam.prjStructure.HardwareSim;

    for (let i = 0; i < HDLparam.length; i++) {
        const element = HDLparam[i];
        if (element.modulePath.includes(simPath)) {
            simHDLparam.push(element);
        }
    }

    simTopElementList = getTopElement(simHDLparam, currentTbTopElement);
    let len = simTopElementList.length;
    // 如果没有sim文件则直接退出
    if (len == 0) {
        return simTopElementList;
    }
    if (process.opeParam.simTopModule.name == '') {
        // 如果没有就将第一个设置为当前顶层
        process.opeParam.simTopModule = {
        ...
        for (let index = 0; index < len; index++) {
            ...
            if (process.opeParam.simTopModule.name == simTopElement.type &&
                process.opeParam.simTopModule.path == simTopElement.fsPath) {
                // 如果在第i个为当前顶层则将其删除
        ...
        if (isExsit) {
            let TopElement = {
                "mode"   : "sim",
                "name"   : "",
                "type"   : process.opeParam.simTopModule.name,
                "fsPath" : process.opeParam.simTopModule.path
            };
            // 直接插入到顶层数组的第一个
