...
class Parser extends base_parser.base_parser {
    ...
    get_module(tree, lines, HDLSymbol, document) {
        ...
        let generates = this.get_generates(tree, lines, module_name[0].text, HDLSymbol, document);
        let processes = this.get_processes(tree, lines, module_name[0].text, HDLSymbol, document, ["generate_region"]);
        let functions = this.get_functions(tree, lines, module_name[0].text, HDLSymbol, document, ["generate_region"]);
        let constants = this.get_constants(tree, lines, module_name[0].text, HDLSymbol, document, ["generate_region"]);
    ...
    get_generates(tree, lines, parent, HDLSymbol, document, ignore_title) {
        ...
        for (let x = 0; x < generateArr.length; ++x) {
            ...
            HDLSymbol.symbolsWithTreeSitter.push(symbolInfo);
