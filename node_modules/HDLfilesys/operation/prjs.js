        ...
        if (fs.statSync(path).isDirectory()) {       
            ...
            for (let i = 0; i < HDLExtensions.length; i++) {
                ...
                if (fspath.extname(path).toLowerCase() === extname) {
                    // vscode.Uri.file(path.replace("//","/").replace(/\\/g,"\/"))
...
function getLibParam(opeParam) {
    ...
    if (files.isHasAttr(opeParam.prjInfo,"HardwareLIB.Common")) {
        for (let i = 0; i < opeParam.prjInfo.HardwareLIB.Common.length; i++) {
            const element = opeParam.prjInfo.HardwareLIB.Common[i];
            refreshHDLFiles(`${opeParam.rootPath}/../lib/src_lib/Hardware/${element}`,libFileList);
        }
    }
    if (files.isHasAttr(opeParam.prjInfo,"HardwareLIB.Customer")) {
        if (fs.existsSync(customerLibPath)) {            
            for (let i = 0; i < opeParam.prjInfo.HardwareLIB.Customer.length; i++) {
                const element = opeParam.prjInfo.HardwareLIB.Customer[i];
                refreshHDLFiles(`${customerLibPath}/${element}`,libFileList);
            }
        } else {
    ...
    for(var i = 0, l = libFileList.length; i < l; i++) {
        for(var j = i + 1; j < l; j++)
        if (libFileList[i] === libFileList[j]) j = ++i;
        array.push(libFileList[i]);
    }
    libFileList = array;
    if (files.isHasValue(opeParam.prjInfo,"HardwareLIB.State","real")) {
        //TODO 删除在lib文件夹下却不在lib列表中的文件
        let LibPath = '';
        if (opeParam.prjInfo.SOC_MODE.soc != "none") {
            LibPath = opeParam.workspacePath + "user/Hardware/src/lib/";
        } else {
            LibPath = opeParam.workspacePath + "user/src/lib/";
        }
        let currentLibFileList = [];
        if (refreshHDLFiles(LibPath,currentLibFileList) != 0) {
            for (let i = 0; i < currentLibFileList.length; i++) {
                const currentLibFile = currentLibFileList[i];
                let exist = 0;
                for (let i = 0; i < libFileList.length; i++) {
                    const libFile = libFileList[i];
                ...
                if (!exist) {
                    fs.unlinkSync(currentLibFile.replace("//","/").replace(/\\/g,"\/"));
                }
            }
        }
        for (let i = 0; i < libFileList.length; i++) {
            const element = libFileList[i];
            let src  = element.replace("//","/").replace(/\\/g,"\/");
        ...
        libFileList = [];
    } else if (files.isHasValue(opeParam.prjInfo,"HardwareLIB.State","virtual")) {
        let dist = '';
        if (opeParam.prjInfo.SOC_MODE.soc != "none") {
