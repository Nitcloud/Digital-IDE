...
class indexer {
     ...
     * @param {Array} FileList 需要解析的文件列表
     */
    async build_index(FileList) {
        ...
        vscode.window.withProgress({
            ...
            title: "The progress of parse HDL Files...",
            cancellable: true
        }, async (progress) => {
            // 初始化进度
            await Promise.all(FileList.map((path, index) => {
                let increment = (index / FileList.length)*100;
                try {
                    progress.report({ increment: increment, message: "parsing..." });
                    // console.log(`${index}\n@ ${path}`);
                    // if (index == 34) {
                    //     console.log();
                    // }
    ...
    processFile(path) {
        ...
        switch (languageId) {
            case "verilog":
            case "systemverilog":    
                this.HDLparam = vlogParser.getFileParam(path, option, this.HDLparam);
            break;

            case "vhdl":    
                this.HDLparam = vhdlParser.getFileParam(path, option, this.HDLparam);
            break;
