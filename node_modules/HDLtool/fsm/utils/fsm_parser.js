...
class Parser_fsm_base {
    ...
    json_to_svg(fsm_json) {
        ...
        let svg;
        try {
            svg = render(stmcat, { outputType: "svg" });
        }
    ...
    search_multiple_in_tree(element, matching_title, ignore_title) {
        var arr_match = [];
        function recursive_searchTree(element, matching_title, ignore_title) {
            ...
            if (type === matching_title) {
                ...
                var i;
                var result = null;
                if (ignore_title != undefined && ignore_title.includes(type)) {
                    return null;
                }
                for (i = 0; result === null && i < element.childCount; i++) {
                    result = recursive_searchTree(element.child(i), matching_title, ignore_title);
                }
                return result;
            }
            return null;
        }
        recursive_searchTree(element, matching_title, ignore_title);
        return arr_match;
    }
...
class Paser_fsm_verilog extends Parser_fsm_base {
    ...
    async get_svg_sm(code) {
        ...
        let Lang = await Language.load("../resources/tree-sitter/tree-sitter-verilog.wasm");
    ...
    get_process_info(proc) {
        ...
        for (let i = 0; i < case_statements.length; ++i) {
            ...
            let p_info = {
                'description': description.replace('ffsm_extract', ''),
                'name': '',
                'state_variable_name': '',
                'states': []
            };
...
class Paser_fsm_vhdl extends Parser_fsm_base {
    ...
    async get_svg_sm(code) {
        ...
        let Lang = await Language.load("../resources/tree-sitter/tree-sitter-vhdl.wasm");
