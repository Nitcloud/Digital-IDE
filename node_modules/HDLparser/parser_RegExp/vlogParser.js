...
function get_ports(text, document, parent, offset, HDLSymbol) {
    ...
    while (1) {
        ...
        let portInfo = get_data_info(text, item.lastIndex, ["input", "output", "inout"]);
        item.lastIndex = portInfo.last + offset;
...
function get_signals(text, document, parent, offset, HDLSymbol) {
    ...
    while (1) {
        ...
        let signalInfo = get_data_info(text, item.lastIndex, ["reg","wire","logic"]);
        item.lastIndex = signalInfo.last + offset;
...
function get_parameter(text, document, parent, offset, HDLSymbol) {
    ...
    while (1) {
        ...
        let paramInfo = get_data_info(text, item.lastIndex, ["parameter"]);
        item.lastIndex = paramInfo.last + offset;
...
function get_constants(text, document, parent, offset, HDLSymbol) {
    ...
    while (1) {
        ...
        let constantInfo = get_data_info(text, item.lastIndex, ["localparam","int","char","float","integer"]);
        item.lastIndex = constantInfo.last + offset;
