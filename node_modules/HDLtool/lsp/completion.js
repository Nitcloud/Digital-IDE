...
class vlogCompletion {
    ...
    provideCompletionItems(document, position) {
        return new Promise((resolve, reject) => {
            ...
            let guideIndex = {
                "startIndex" : guide_index,
                "lastIndex"  : guide_index,
            }
            ...
            let illegalRange = parser.vlogParser.get_comment_index(text);
            for (let index = 0; index < illegalRange.length; index++) {
                const range = illegalRange[index];
                if (utils.ensureInclude(range, guideIndex)) {
                    resolve([]);
                }
            }
            ...
            switch (guide) {
                case '.' :
                    let item = this.getInstance(text, path, guideIndex);
                    let defines = utils.getInstDefine(guideIndex, item);
                    resolve(this.defines_to_items(defines, item.instModPath));
                break;
     ...
     */
    getInstance(text, path, wordIndex) {
        let modules = [];
        let option = {
            text : text,
            isFast : true,
        ...
        parser.vlogParser.get_modules(path, option, modules);

        for (let index = 0; index < modules.length; index++) {
            const module = modules[index];
            for (let index = 0; index < module.instances.length; index++) {
                const instance = module.instances[index];
                if (!utils.ensureInclude(instance, wordIndex)) {
                    continue;
                }
                return instance;
            }
     ...
     */
    defines_to_items(defines, detail){
        let items = [];
