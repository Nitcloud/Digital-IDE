...
class iverilogSimulate {
        ...
        for (let i = 0; i < HDLparam.length; i++) {
            ...
            if (unitModule.moduleName == module) {
                for (let i = 0; i < unitModule.instances.length; i++) {
                    ...
                    this.getInstmoduleFilePathList(HDLparam, instanceModule.instModule, instmoduleFilePathList);
                }
                break;
            }         
        }
    ...
    simulate(uri, HDLparam) {
        ...
        parser.utils.selectCurrentFileModule(HDLparam, docPath).then((selectModule) => {
            if (selectModule != null) {
                this.execSimulation(docPath, selectModule.moduleName, HDLparam);
            }
        });      
    }
    ...
    execSimulation(docPath, module, HDLparam) {
        ...
        let includeList = parser.vlogParser.get_include(fs.readFileSync(docPath, "utf-8"));
        for (let i = 0; i < includeList.length; i++) {
            let pathElement = includeList[i];
            if (!fspath.isAbsolute(pathElement)) {
                let parent = fspath.dirname(docPath);
                pathElement = fspath.resolve(parent, pathElement)
            }
            pathElement = pathElement.replace(/\\/g,"\/").replace("//","/");
            let index = instmoduleFilePathList.indexOf(pathElement);
            if (index != -1) {
