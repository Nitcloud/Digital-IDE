...
var kernelOperation = {
    ...
    exportJson : function (mode) {
        if (mode === "Before Behavioral Synth") {
            this.exec(this.cmd[0]);
        }
        if (mode === "After Behavioral Synth") {
            this.exec(this.cmd[1]);
        }
        if (mode === "After RTL Synth") {
            this.exec(this.cmd[2]);
        }
        return virtualFileSystem.readFileToText('output.json');
    },

    synthXilinx(topModule, edif_path, blif_path, arg) {
