...
class FileSystemProvider {
    ...
    getChildren(element) {
        ...
        if (element) {
            switch (element.type) {
                case "src"  : return parser.simParser.getSrcTopElement(this.opeParam, parser.simParser.getTopElement(this.HDLparam));
                // case "Data"      : return this.getDataTopElement();
                case "sim"  : return parser.simParser.getTbTopElement(this.opeParam, parser.simParser.getTopElement(this.HDLparam));
    ...
    getTreeItem(element) {
        ...
        if (element.name == "" || element.name == undefined) {
            iconPath = (element.type == "src") ? "src" : "top";
            if (element.type == "sim") {
                iconPath = "sim";
            }
            if ((element.type != "src" && element.type != "sim") && element.fspath.indexOf(this.opeParam.workspacePath) == -1) {
                iconPath = "lib";
            ...
            if (fs.existsSync(element.fspath)) {                
                if (element.fspath.indexOf(this.opeParam.workspacePath) != -1) {                
                    if (this.getLanguageId(element.fspath) == "vlog") {
                        iconPath = "verilog";
                    } else if (this.getLanguageId(element.fspath) == "vhdl") {
                        iconPath = "vhdl";
                    }
                } else {
                    iconPath = "lib";
    ...
    getLanguageId(path) {
        ...
        for (let i = 0; i < vhdlExtensions.length; i++) {
            const element = vhdlExtensions[i];
            if (value === element) {
                return "vhdl";
            }
        }
...
class FileExplorer {
    constructor(HDLparam, opeParam) {
        ...
        this.fileExplorer = vscode.window.registerTreeDataProvider('TOOL.file_tree', this.treeDataProvider);
