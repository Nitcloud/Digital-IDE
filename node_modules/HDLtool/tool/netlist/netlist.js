...
class showNetlist {
    ...
    open_viewer(uri, opeParam) {
        ...
        if (filesys.files.isHasAttr(uri, "name")) {
            ...
            parser.utils.selectCurrentFileModule(this.indexer.HDLparam, docPath).then((selectModule) => {
    ...
    createViewer(opeParam) {
        ...
        this.panel.webview.onDidReceiveMessage(message => {
            switch (message.command) {
                ...
                break;
                case 'exec':
                    this.sendPrjNetlist(message.mode);
                break;
            }
    ...
    async getPrjNetlist(module, opeParam) {
        // 获取工程依赖
        let dependenceFilePathList = parser.utils.getModuleDependence(this.indexer.HDLparam, module, "all");
        ...
        // 将需要综合的文件进行导入内核
        for (let index = 0; index < dependenceFilePathList.include.length; index++) {
            const element = dependenceFilePathList.include[index];
            this.synth.vfs.writeFileFormPath(element, element);
        }
        this.synth.ope.loadFileFromPath(dependenceFilePathList.inst);
        this.synth.ope.loadFileFromPath([module.path]);
    ...
    sendPrjNetlist(mode) {
        // 导出模块的netlist  
        let netlist = this.synth.ope.exportJson(mode);
