...
class showNetlist {
    ...
    async sendPrjNetlist(module) {
        ...
        // 向内核中导入工程
        let synth = await kernel.launch();

        // 将执行过程中的日志输出到webview
        // synth.ope.setMessageCallback((message, type) => {
        //     this.panel.webview.postMessage({ command: type, message: message});
        // });

        // 将需要综合的文件进行导入内核
        synth.ope.loadFileDirect(dependenceFilePathList.inst);
        synth.ope.loadFileDirect(dependenceFilePathList.include);
        synth.ope.loadFileDirect([module.path]);
