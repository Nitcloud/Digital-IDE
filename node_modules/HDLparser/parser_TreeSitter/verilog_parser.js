...
class Parser extends base_parser.base_parser {
    ...
    getFileParam(path, HDLSymbol, HDLparam, comment_symbol) {
        ...
        try {
            ...
            for (let index = 0; index < moduleArray.length; index++) {
                ...
                let structure = this.get_module(elementChildren, lines, HDLSymbol, path)
                if (HDLparam != null) {
    ...
    //--------module block--------//
    get_module(tree, lines, HDLSymbol, fspath) {
        ...
        let symbolInfo = HDLSymbol.setSymbolInformationWithTreeSitter(item, null);
        HDLSymbol.symbols.push(symbolInfo);

        let generates = this.get_generates(tree, lines, module_name[0].text, HDLSymbol);
        let processes = this.get_processes(tree, lines, module_name[0].text, HDLSymbol, ["generate_region"]);
        let functions = this.get_functions(tree, lines, module_name[0].text, HDLSymbol, ["generate_region"]);
        let constants = this.get_constants(tree, lines, module_name[0].text, HDLSymbol, ["generate_region"]);
        let generics  = this.get_generics(tree, lines, module_name[0].text, HDLSymbol, ["generate_region"]);
        let signals   = this.get_signals(tree, lines, module_name[0].text, HDLSymbol, ["generate_region"]);
        let instance  = this.get_instantiations(tree, lines, module_name[0].text, HDLSymbol, ["generate_region"]);
        let ports     = this.get_ports(tree, lines, module_name[0].text, HDLSymbol);
    ...
    //--------generate block--------//
    get_generates(tree, lines, parent, HDLSymbol, ignore_title) {
        let items = [];
        let item = {};
        //Inputs
        ...
        for (let x = 0; x < generateArr.length; ++x) {
            item = {
                ...
                "type": "generate",
                "startPosition": generateArr[x].startPosition,
                "endPosition" : generateArr[x].endPosition
            };
            let symbolInfo = HDLSymbol.setSymbolInformationWithTreeSitter(item, parent);
            HDLSymbol.symbols.push(symbolInfo);
            this.get_instantiations(generateArr[x], lines, item.name, HDLSymbol);
            this.get_processes(generateArr[x], lines, item.name, HDLSymbol);
            this.get_functions(generateArr[x], lines, item.name, HDLSymbol);
            this.get_constants(generateArr[x], lines, item.name, HDLSymbol);
            this.get_generics(generateArr[x], lines, item.name, HDLSymbol);
            this.get_signals(generateArr[x], lines, item.name, HDLSymbol);
            items.push(item);
        }
        return items;
    }
    ...
    get_functions(tree, lines, parent, HDLSymbol, ignore_title) {
        let items = [];
        let item  = {};
        ...
        for (let x = 0; x < functionArr.length; ++x) {
            ...
            item = {
                "name": function_name,
                "type": "function",
                "startPosition": functionArr[x].startPosition,
                "endPosition" : functionArr[x].endPosition,
            ...
            for (let x = 0; x < tfPortArr.length; ++x) {
                item = {
                    ...
                    "type": "inport",
                    "startPosition": tfPortArr[x].startPosition,
                    "endPosition" : tfPortArr[x].endPosition,
                    "start_line" : tfPortArr[x].startPosition.row
                };
                let symbolInfo = HDLSymbol.setSymbolInformationWithTreeSitter(item, function_name);
                HDLSymbol.symbols.push(symbolInfo);
            }
        }
        return items;
    ...
    get_tasks(tree, lines, parent, HDLSymbol) {
        let items = [];
        let item = {};
        //Inputs
        ...
        for (let x = 0; x < arr.length; ++x) {
            item = {
                ...
                "type": "task",
                "startPosition": arr[x].startPosition,
                "endPosition": arr[x].endPosition,
    ...
    get_processes(tree, lines, parent, HDLSymbol, ignore_title) {
        let items = [];
        //Inputs
        let processArr = this.search_multiple_in_tree(tree, 'always_construct', ignore_title);
        for (let x = 0; x < processArr.length; ++x) {
            items.push(this.get_processes_unit(processArr[x], lines, parent, HDLSymbol));
        }
        return items;
    }

    get_processes_unit(tree, lines, parent, HDLSymbol) {
        let item = {};
    ...
    get_instantiations(tree, lines, parent, HDLSymbol, ignore_title) {
        let items = [];
        let item = {};
        //Inputs
    ...
    get_ports(tree, lines, parent, HDLSymbol, ignore_title) {
        var items = [];
        var element = tree;
        //Inputs
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let symbolInfo = HDLSymbol.setSymbolInformationWithTreeSitter(element, parent);
            HDLSymbol.symbols.push(symbolInfo);
        }
        ...
        for (var x = 0; x < inputs.length; ++x) {
            ...
            for (var i = 0; i < port_name.length; i++) {
                ...
                item = {
                    ...
                    "default_value": "",
                    "startPosition": inputs[x].startPosition,
                    "endPosition" : inputs[x].endPosition
                };
                items.push(item);
            }
        }
    ...
    get_generics(tree, lines, parent, HDLSymbol, ignore_title) {
        let items = [];
        let item = {};
        let arr = [];
    ...
    get_constants(tree, lines, parent, HDLSymbol, ignore_title) {
        let items = [];
        let item  = {};
        //Inputs
        ...
        for (let x = 0; x < localParamArr.length; ++x) {
            ...
            for (let i = 0; i < arr.length; ++i) {
                item = {
                    ...
                    "startPosition": localParamArr[i].startPosition,
                    "endPosition" : localParamArr[i].endPosition
                };
                let symbolInfo = HDLSymbol.setSymbolInformationWithTreeSitter(item, parent);
                HDLSymbol.symbols.push(symbolInfo);
                items.push(item);
    ...
    //--------signal element--------//
    get_signals(tree, lines, parent, HDLSymbol, ignore_title) {
        let items = [];
        //Inputs
