...
function Overwrite_tb(opeParam) {
    ...
    vscode.window.showQuickPick(['vlog','vhdl'],{placeHolder:"Which type you want to Overwrite?"}).then(select => {
        if (select == "vlog") {
            tbSourcePath = `${opeParam.rootPath}/lib/com/Hardware/testbench.v`;
        } else if (select == "vhdl") {
            tbSourcePath = `${opeParam.rootPath}/lib/com/Hardware/testbench.vhd`;
        }
        const options = {
            preview: false,
...
function insertInstContent(content, tbFilePath, rootPath) {
    ...
    if (fs.existsSync(tbFilePath)) {
        ...
        let tbSource = `${rootPath}/lib/com/Hardware`;
