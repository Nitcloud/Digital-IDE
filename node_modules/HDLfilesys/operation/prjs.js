...
function generatetbFile(tbFilePath,rootPath) {
    if (!fs.existsSync(tbFilePath)) {
        let tb_template = fs.readFileSync(`${rootPath}/.TOOL/.Data/testbench.v`, 'utf8');
...
function refreshPrjFolder(opeParam) {
    if (opeParam.propertyPath != "") {        
        ...
        if (prj_info.SOC_MODE.soc == "none") {
            ...
            generatetbFile(`${opeParam.workspacePath}user/sim/testbench.v`,opeParam.rootPath);
        } else {
            ...
            generatetbFile(`${opeParam.workspacePath}user/Hardware/sim/testbench.v`,opeParam.rootPath);
        }
    }
}
...
function refreshPrjInfo(opeParam) {
    if (opeParam.propertyPath != "") {
        let prj_param = files.pullJsonInfo(opeParam.propertyPath);
        ...
        fs.writeFileSync(`${opeParam.rootPath}/.TOOL/CONFIG`,CONFIG_content);
        return prj_param;
    }
}
...
function delDevice(opeParam) {
    let propertyParam = files.pullJsonInfo(`${opeParam.rootPath}/../property.json`);
    vscode.window.showQuickPick(propertyParam.properties.Device.enum).then(selection => {
        if (!selection) {
            return;
        }
        for(var index = 0; index < propertyParam.properties.Device.enum.length; index++){
            if(selection == propertyParam.properties.Device.enum[index]){
