...
class FileSystemProvider {
    ...
    getChildren(element) {
        ...
        if (element) {
            switch (element.type) {
                case "src"  : return parser.utils.getSrcTopElement(this.process, this.indexer.HDLparam);
                // case "Data"      : return this.getDataTopElement();
                case "sim"  : return parser.utils.getSimTopElement(this.process, this.indexer.HDLparam);
                default     : return parser.utils.getChildElement(element.type, this.indexer.HDLparam);
            }
        }
    ...
    getTreeItem(element) {
        ...
        if (element.name == "") {
            ...
            else if (!element.fsPath.includes(this.process.opeParam.workspacePath)) {
                iconPath = "lib";
            ...
            if (fs.existsSync(element.fsPath)) {                
                if (!element.fsPath.includes(this.process.opeParam.workspacePath)) {                
                    iconPath = parser.utils.getLanguageId(element.fsPath);
                } else {
                    iconPath = "lib";
                }
            } else {
        ...
        let fileType = ["vhdl", "systemverilog", "verilog", "lib", "top"]
        ...
        if (iconPath == "top") {
            if (this.process.opeParam.srcTopModule && element.mode == "src") {
                if (this.process.opeParam.srcTopModule.name == element.type &&
                    this.process.opeParam.srcTopModule.path == element.fsPath) {
                    iconPath = "currentSrcTop";
                }
            }
            if (this.process.opeParam.simTopModule && element.mode == "sim") {
                if (this.process.opeParam.simTopModule.name == element.type &&
                    this.process.opeParam.simTopModule.path == element.fsPath) {
...
class hardTreeDataProvider {
    ...
    getCommand(name){
        ...
        switch (name) {
            ...
            case "Gen_Bit"  : cuurentCommand.command = "HARD.Gen_Bit";  break;
