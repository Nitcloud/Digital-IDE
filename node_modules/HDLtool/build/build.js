...
class synthNetlist {
    constructor(context, opeParam, HDLparam) {
        ...
        this.mode = "Before Behavioral Synth";
    }
    ...
    open_viewer(uri) {
        ...
        if (filesys.files.isHasAttr(uri, "type")) {
            if (uri.type != null) {
                let synthPrjInfo = this.getPrjNetlist({
                    name:uri.type,
                    path:docPath
                });
                if (synthPrjInfo !== null) {
                    this.sendNetlist(synthPrjInfo);
                }
            } else {
    ...
    async getPrjNetlist(module) {
        // 获取工程依赖
        let dependenceFilePathList = parser.utils.getModuleDependence(this.HDLparam, module, "all");

        // 向内核中导入工程
        let synth = await kernel.launch(this.opeParam);

        // 将执行过程中的日志输出到webview
        synth.ope.setMessageCallback((message) => {
            this.panel.webview.postMessage({ command: "meaasge", message: message});
        });

        // 将需要综合的文件进行导入内核
        synth.ope.loadFile(dependenceFilePathList.inst, this.opeParam.workspacePath);
        synth.ope.loadFile(dependenceFilePathList.include, this.opeParam.workspacePath);
        synth.ope.loadFile([module.path], this.opeParam.workspacePath);

        // 导出模块的netlist
        let netlist = synth.ope.exportJson(this.mode);
        this.sendNetlist(netlist);
    }
    ...
    sendNetlist(netlist) {
        ...
        this.panel.webview.postMessage({ command: "update", netlist: netlist});
    }
