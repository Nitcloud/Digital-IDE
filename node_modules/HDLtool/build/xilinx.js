...
function Clean(opeParam) {
    ...
    if (opeParam.propertyPath != "") {
        let fileList = fs.readdirSync(opeParam.workspacePath);
        for (let i = 0; i < fileList.length; i++) {
            const element = fileList[i];
            if (element != ".vscode" && 
                element != "user" && 
                fs.statSync(`${opeParam.workspacePath}${element}`).isDirectory()) {
                filesys.dirs.deleteDir(`${opeParam.workspacePath}${element}`);
            }
        }        
