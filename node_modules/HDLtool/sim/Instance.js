...
function instantiateModuleInteract(indexer, workspacePath) {
    ...
    parser.utils.selectModule(indexer.HDLparam, workspacePath).then(selectModule => {
...
function instantiateVlogModule(module) {
    ...
    for (let i = 0; i < module.ports.length; i++) {
        if (module.ports[i].type == "output") {
            ...
            widthElement = widthElement.replace('(','[').replace(')',']').replace(/downto/mgi,':') + ' '.repeat(padding);
            initPort += `wire ${widthElement}\t${module.ports[i].portName};\n`;
        }
    }

    let paramString = ``;
    if (module.parameter.length > 0) {
        paramString = `#(\n${instantiateVlogParam(module.parameter)})\n`;
    }
    let portString = ``;
...
function instantiateVhdlModule(module) {
    ...
    if (module.param.length > 0) {
        paramString = `generic map(\n${instantiateVhdlParam(module.parameter)})\n`;
    }
    let portString = ``;
