...
function generateBootFile(opeParam) {
    let BootInfo = {
        ...
        "inside_folder"  : `${opeParam.rootPath}/.TOOL/Xilinx/BOOT`,
        "output_path"    : `${opeParam.rootPath}/.TOOL/Xilinx/BOOT`,
        "elf_path"    : '',
        "bit_path"    : '',
        "fsbl_path"   : ''
    };
    ...
    BootInfo.fsbl_path = generatefsblPath(opeParam);
    if (fs.existsSync(BootInfo.BOOT_folder)) {
        BootInfo.bit_path = generateBitPath(BootInfo.outside_folder);
        BootInfo.elf_path = generateElfPath(BootInfo.outside_folder);
    }
    else {
        BootInfo.bit_path = generateBitPath(BootInfo.inside_folder);
        BootInfo.elf_path = generateElfPath(BootInfo.inside_folder);
    }
...
function generatefsblPath(opeParam) {
    let fsbl_path = "";
    if (fs.existsSync(`${opeParam.workspacePath}user/BOOT/fsbl.elf`)) {
        fsbl_path = `\t[bootloader]${opeParam.workspacePath}user/BOOT/fsbl.elf\n`;
    }
    else {
        fsbl_path = "\t[bootloader]" + `${opeParam.rootPath}/.TOOL/Xilinx/BOOT` + "/fsbl.elf\n";
    }
    return fsbl_path;
}

async function generateBitPath(bit_path) {
...
async function generateElfPath(elf_path) {
