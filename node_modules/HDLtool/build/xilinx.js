...
class xilinxOperation {
    ...
    refresh() {
        ...
        if (filesys.files.isExist(xip_repo_path)) {
        ...
        if (filesys.files.isExist(mref_path)) {
    ...
    generateBit() {
        ...
        if (designMode && (designMode != "none")) {
            if (filesys.files.isExist(sysdefPath)) {
    ...
    async boot() {
        ...
        output_context += "}";
        let result = filesys.files.writeFile(BootInfo.outputPath, output_context);
        if (!result) {
            return null;
        }
    ...
    async getfsblPath(outsidePath, insidePath) {
        let paths = filesys.files.filter(outsidePath, (element) => {
            if(filesys.paths.extname(element) != ".elf"){
                return null;
            }
    ...
    pickElfFile(path) {
        return filesys.files.filter(path, (element) => {
            if(filesys.paths.extname(element) != ".elf"){
                return null;
            }
    ...
    Add_bd(uri) {
        ...
        if (!filesys.files.isExist(bd_folder)) {
